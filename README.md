...
| 2b *(Savio)* | `ld_decay.sh` | Computes pairwise LD (r²) up to 2 Mb apart from the genome-wide merged VCF, thinned to 1 SNP per 2kb for computational tractability. Outputs `ld_decay.geno.ld`. |
| 2c *(Savio)* | `bin_ld_decay.sh` | Bins the pairwise LD output into 50kb distance bins, producing the LD-decay curve used to justify the 250 kb window size (Methods: r² = 0.63 at <50kb, 0.26 at 2Mb). Outputs `ld_decay_binned.csv`. |
...
#### Notes on the genome-wide PCA robustness check
...
| `ld_decay_binned.csv` | Pairwise LD (r²) binned by physical distance, used to justify the 250 kb window size (Methods) |
...
- `ape` v5.8, `phangorn` — neighbour-joining tree construction
- `SNPRelate` v1.38 — sliding-window PCA
- `vegan` v2.7.5 — Mantel tests (genomic distance vs. LD50; CDS-restricted vs. genome-wide concordance)
- `mclust`, `diptest`, `cluster` — genomic substructure diagnostics (Gaussian mixture model/BIC, Hartigan's dip test, silhouette analysis)
- `data.table` — fast reading of genome-wide genotype tables
- `readxl` — reading raw life-history/eclosion tracking spreadsheets
- `dplyr`, `tibble` — data manipulation throughout
- `emmeans`, `multcomp` — Tukey post-hoc comparisons (Figure 1)
- `ggplot2`, `patchwork`, `ggrepel` — figures
- `gridExtra`, `grid` — table and layout rendering

Computationally intensive analyses (sliding-window PCA, WZA, genome-wide PCA, LD-decay) were run on the Savio HPC cluster at UC Berkeley under account `fc_sirmodel`.
